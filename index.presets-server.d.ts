import './index.plugin-animation.d.ts'
import './index.plugin-chat.d.ts'
import './index.plugin-entities-app.d.ts'
import './index.plugin-entities-player.d.ts'
import './index.plugin-evm.d.ts'
import './index.plugin-hyperliquid.d.ts'
import './index.plugin-loader-server.d.ts'
import './index.plugin-loader-server-handlers.d.ts'
import './index.plugin-livekit-server.d.ts'
import './index.plugin-network-server.d.ts'
import './index.plugin-nodes.d.ts'
import './index.plugin-runtime-server.d.ts'
import './index.plugin-spatial.d.ts'
import './index.plugin-stage.d.ts'
import './index.plugin-storage.d.ts'
import './index.plugin-view.d.ts'
import type { WorldPlugin, WorldPreset } from './index.plugins.d.ts'

export declare const serverRuntimePlugin: WorldPlugin
export declare const serverPreset: WorldPreset
export declare function createServerWorld(options?: { plugins?: any }): any
