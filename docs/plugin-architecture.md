...
The old root `extras` bucket has been removed. Shared primitives now live behind named kernel contracts: `math` for the patched Three.js facade and rotation/math helpers, `layers` for collision/camera layer masks, `permissions` for rank constants, and `diagnostics` for shared warnings. Browser-only helpers live with the browser plugin support code, and server-specific runtime helpers live with the server package instead of the kernel root.
...
Feature-owned vendor helpers are no longer kept under `@gamedev/core`. CSM lives with the environment plugin, the server-side GLTF parser lives with the loader handler plugin, stats-gl lives with the stats plugin, and the shared custom shader material wrapper lives under plugin rendering support. The unused vendored three-vrm bundle was removed; the client loader uses the package dependency directly when the first-party VRM loader plugin is included.
...
The first-party WebSocket packet table lives with the network plugin because it contains network/admin/log/AI/LiveKit message names, not generic kernel state. The server socket wrapper lives with the server network plugin that owns connection lifecycle.
...
Admin client tooling, the client/admin network bridges, admin XR no-op, builder client/admin tooling, and builder-owned built-in app templates live outside `@gamedev/core`. The templates are exported from `gamedev/plugins/builder/builtins` and are intentionally no longer part of the core kernel surface.
