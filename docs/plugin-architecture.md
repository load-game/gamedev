...
AI, EVM, and Hyperliquid are first-party feature plugins outside `@gamedev/core`. Their public entrypoints remain `gamedev/plugins/ai/client`, `gamedev/plugins/ai/server`, `gamedev/plugins/evm`, and `gamedev/plugins/hyperliquid`, but their implementations live under `packages/plugins` rather than the kernel package.
