...
Admin client tooling, the admin network bridge, builder client/admin tooling, and builder-owned built-in app templates live outside `@gamedev/core`. The templates are exported from `gamedev/plugins/builder/builtins` and are intentionally no longer part of the core kernel surface.
