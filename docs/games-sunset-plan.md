...
- [ ] P1-01 (M, `runtime`): Narrow `src/server/forwardedPrefix.js` back to `/worlds/*` handling only.
...
- [ ] P2-01 (M, `runtime`): Restore strict `WORLD_ID` vs DB `worldId` validation in `src/core/systems/ServerNetwork.js`.
