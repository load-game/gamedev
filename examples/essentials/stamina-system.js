...
world.on(`stamina:consume:${playerId}`, ({ amount, requestId, source }) => {
  ...
  if (config.debugMode) {
    debugLog('CONSUME REQUEST - amount:', amount, 'oldStamina:', oldStamina.toFixed(1), 'source:', source || 'unknown')
  }
  ...
  if (unlimitedStamina) {
    ...
    if (config.debugMode) {
      debugLog('UNLIMITED MODE - returning same stamina')
    }
  } else {
    ...
    if (config.debugMode) {
      debugLog('CONSUMED - actualAmount:', actualAmount.toFixed(3), 'newStamina:', stamina.toFixed(1))
    }
  }
...
// Handle try-consume requests (all-or-nothing) - only checks availability, does NOT deduct
