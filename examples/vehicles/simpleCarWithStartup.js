...
app.configure([
  ...
  // Seat Position Adjustments
  {
    key: 'seat1XOffset',
    type: 'number',
    label: 'Seat1 X Offset',
    initial: 0,
    dp: 2,
    step: 0.01,
    hint: 'Horizontal position offset for Seat1 (driver seat) in meters.',
  },
  {
    key: 'seat1YOffset',
    type: 'number',
...
const seat2XOffset = props.seat2XOffset || 0
const seat2YOffset = props.seat2YOffset || 0
const seat2ZOffset = props.seat2ZOffset || 0
...
function stopEngine() {
  ...
  for (const audio of Object.values(audioSources)) {
    safeExecute(() => {
      // Check if audio exists, has stop method, AND is still attached to scene
...
function setupTouchControls() {
  ...
  safeExecute(() => {
    ...
    // Gas button - positioned below exit button
    ...
    app.add(gasButton)
    driverPassengerMobileControls.push(gasButton)
    // Brake button - positioned next to gas button
    const brakeButton = app.create('ui', {
      space: 'screen',
      width: 40,
      height: 40,
      backgroundColor: 'rgba(200, 50, 50, 0.3)',
      ...
      cursor: 'pointer',
      onPointerDown: () => {
        brakeButton.backgroundColor = 'rgba(200, 50, 50, 0.5)'
        accelInput = -1
        brakeInput = true
      },
      onPointerUp: () => {
        brakeButton.backgroundColor = 'rgba(200, 50, 50, 0.3)'
        accelInput = 0
        brakeInput = false
      },
      alignItems: 'center',
    ...
    const brakeLabel = app.create('uitext', {
      value: 'BRAKE',
      color: 'white',
      fontSize: 8,
      fontWeight: 'bold',
    })
    brakeButton.add(brakeLabel)
    // app.add(brakeButton)
    // driverPassengerMobileControls.push(brakeButton)

    // Hop/Drift button - positioned where brake button was
    const hopButton = app.create('ui', {
      space: 'screen',
      width: 40,
      height: 40,
      backgroundColor: 'rgba(150, 100, 200, 0.3)', // Purple for hop
      borderRadius: 20,
      pivot: 'top-right',
      position: [1, 1],
      offset: [-145, -210],
    ...
    app.add(hopButton)
    driverPassengerMobileControls.push(hopButton)

    // Setup touch panning for camera control on right side of screen
    touchPan = null
      ...
      case CAMERA_MODES.COCKPIT:
        if (seatNodes[0]) {
          ...
          if (car.matrixWorld) {
            ...
            control.camera.position.add(car.position)
          }
  ...
  function init(_state_) {
    ...
    const seats = seatNodes.map((node, seatIdx) => {
      ...
      if (seatIdx === 0) {
        ...
        anchor.position.x = seat2XOffset
        anchor.position.y = seat2YOffset
        anchor.position.z = seat2ZOffset
      }
      node.add(anchor)
  ...
  return {
    fixedUpdate(delta) {
      ...
      if ((handbrakeInput || isSlipping) && accelInput) {
        power = Math.min(power + 1 * delta, 1.5) // Cap power accumulation during slip
      } else if (isSlipping) {
        power -= 0.8 * delta // Faster decay when in slip mode
      } else {
        power -= 0.4 * delta // Normal decay when not slipping
      }
      ...
      if (!isSlipping && power > 0.8 && power > roadPower && !handbrakeInput) {
        ...
        isSlipping = false // Quicker exit: lower power threshold OR centered steering
      }
    ...
    update(delta) {
      ...
      if (control) {
        ...
        // Hop input (spacebar on PC, button on mobile)
        ...
        // Handbrake is now hop button on PC
        handbrakeInput = false
