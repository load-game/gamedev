// ULTIMATE Simple Go Kart - Enhanced with Exhaust & Steering Wheel
// Features: Intuitive controls, hop/drift mechanics, multi-platform support
// NEW: Left/Right exhaust particles, Steering wheel bone rotation
...
app.configure([
  ...
  // Handbrake / E-brake
  {
    ...
    initial: 8,
    min: 0,
    hint: 'E-brake force for drifting (Shift key / Mobile button)',
  },
  // Suspension
  {
    key: 'springStrength',
  ...
  // Seat Position Adjustments
  {
    ...
    dp: 2,
    step: 0.01,
    hint: 'Horizontal position offset for driver seat',
  },
  {
    key: 'seat1YOffset',
    type: 'number',
    ...
    dp: 2,
    step: 0.01,
    hint: 'Vertical position offset for driver seat',
  },
  {
    key: 'seat1ZOffset',
    type: 'number',
    ...
    dp: 2,
    step: 0.01,
    hint: 'Forward/back position offset for driver seat',
  },
  // Audio files
  {
    key: 'engineIdle',
  ...
  // Animation bone name
  {
    key: 'animationBone',
    type: 'text',
    label: 'Animation Bone Name',
    initial: 'Engine',
    hint: 'Bone that plays ON/OFF animations (ON loops as idle, OFF is one-shot)',
  },
  // Particle textures
  {
    key: 'treadmark',
...
const handbrakeForce = props.handbrakeForce || 8
...
const seat1XOffset = props.seat1XOffset || 0
const seat1YOffset = props.seat1YOffset || 0
const seat1ZOffset = props.seat1ZOffset || 0
...
// Note: Animations are played on the SkinnedMesh (body), not individual bones
// This line is present but animations are triggered on body
const animationBone = safeExecute(() => body?.getBone(props.animationBone || 'Engine'))

// NEW: Get exhaust empties and steering wheel bone
const leftExhaust = safeExecute(() => app.get('LeftExhaust'))
const rightExhaust = safeExecute(() => app.get('RightExhaust'))
const steeringWheel = safeExecute(() => body?.getBone('SteeringWheel'))

// Camera system - removed KartCam fixed mode, using simple follow camera
...
// NEW: Left and Right Exhaust Particle Systems
let leftExhaustParticles = null
let rightExhaustParticles = null

if (enableParticles) {
  // Create visible exhaust at rear of car (fallback position)
  // Positioned high and visible for debugging
  const defaultExhaustPos = new Vector3(0, 0.8, 1.5) // Rear of car, higher up

  // Test exhaust at car center if no exhaust nodes found
  if (!leftExhaust || !rightExhaust) {
    console.log('KART: Exhaust nodes not found, using fallback position at rear')
    const testExhaust = app.create('particles', {
      emitting: false,
      shape: ['cone', 0.15, 0.4, 12],
      rate: 0,
      life: '1.5~2.5',
      size: '0.6~1.2',
      speed: '1.5~3',
      ...
      alpha: '0.6~0.9',
      alphaOverLife: '0,0.9|0.2,0.7|1,0',
      sizeOverLife: '0,0.6|0.5,1|1,1.8',
      image: exhaustUrl,
    })
    testExhaust.position.copy(defaultExhaustPos)
    car.add(testExhaust)
    // Use test exhaust for both variables
    leftExhaustParticles = testExhaust
    rightExhaustParticles = testExhaust
    console.log('KART: Fallback exhaust created at', defaultExhaustPos)
  } else {
    // Left exhaust particles
    if (leftExhaust) {
      leftExhaustParticles = app.create('particles', {
        emitting: false,
        shape: ['cone', 0.08, 0.2, 8],
        rate: 0,
        life: '1~2',
        size: '0.3~1.0',
        speed: '1~3',
      ...
      leftExhaust.add(leftExhaustParticles)
    }

    // Right exhaust particles
    if (rightExhaust) {
      rightExhaustParticles = app.create('particles', {
        emitting: false,
        shape: ['cone', 0.08, 0.2, 8],
        rate: 0,
        life: '1~2',
      ...
      rightExhaust.add(rightExhaustParticles)
    }
  }
}
...
const info = {
  ...
  read() {
    ...
    this.exhaustEmitting = isEngineRunning && power > 0.2
    this.steeringWheelRotation = steeringWheel ? steeringWheel.rotation.y : 0
  ...
  deserialize(data) {
    ...
    const [px, py, pz, qx, qy, qz, qw, engine, exhaust, steering, w0o, w0t, w0r, w1o, w1t, w1r, w2o, w2t, w2r, w3o, w3t, w3r] = data

    this.position.set(px || 0, py || 0, pz || 0)
    this.quaternion.set(qx || 0, qy || 0, qz || 0, qw !== undefined ? qw : 1)
    this.engineRunning = engine !== undefined ? engine !== 0 : false
    this.exhaustEmitting = exhaust !== undefined ? exhaust !== 0 : false
    this.steeringWheelRotation = steering || 0
  ...
  serialize() {
    return [
      ...
      this.quaternion.z,
      this.quaternion.w,
      this.engineRunning ? 1 : 0,
      this.exhaustEmitting ? 1 : 0,
      this.steeringWheelRotation,
...
// Steering calculation simplified to match simpleCar behavior
function calculateSteeringInput() {
  if (!control) return 0

  let steerValue = 0

  // Keyboard steering (A/D)
  if (control.keyA?.down) steerValue += 1
  if (control.keyD?.down) steerValue -= 1

  return steerValue
}

// NEW: Update steering wheel rotation (only for driver, networked to others)
function updateSteeringWheel() {
  if (!steeringWheel || !isSeated || !control) return

  // Update steering wheel based on current steering input
  // steerAngle is in radians, convert to degrees for bone rotation
  const steerDegrees = steerAngle * (180 / Math.PI)
  steeringWheel.rotation.y = steerDegrees
}

// Simple camera update function - follows car with vertical clamping
function updateCamera(delta) {
  ...
  safeExecute(() => {
    ...
    // Mouse pointer control for camera rotation with vertical clamping
    if (control.pointer && control.pointer.delta) {
      ...
      // Clamp vertical rotation: -15° down, +7.5° up (prevents looking under ground)
      control.camera.rotation.x = clamp(control.camera.rotation.x, -Math.PI / 12, Math.PI / 24)
    } else if (touchPan && touchPan.delta) {
      // Mobile touch panning with vertical clamping
      control.camera.rotation.reorder('YXZ')
...
// Mobile controls setup - EXACTLY like simpleCarWithStartup.js
function setupMobileControls() {
  ...
  safeExecute(() => {
    ...
    // Gas button - exactly like simpleCar
    ...
    // Hop button - exactly like simpleCar
    ...
    // Setup touch panning for camera control on right side of screen - exactly like simpleCar
    touchPan = null

    if (control.on) {
      // Store original handlers
      const originalOnTouchStart = control.onTouchStart
      const originalOnTouchEnd = control.onTouchEnd

      control.on('touchstart', touch => {
        // Call original handler if exists
        if (originalOnTouchStart) originalOnTouchStart(touch)
...
if (world.isClient) {
  ...
  function init(_state_) {
    ...
    anchor.position.x = seat1XOffset
    anchor.position.y = seat1YOffset
    anchor.position.z = seat1ZOffset
    seatNode.add(anchor)
    ...
    app.on('seat', playerId => {
      if (state.sitting === player.id) {
        ...
        // Play engine OFF animation on SkinnedMesh (one-shot)
        safeExecute(() => {
          if (body && body.play) {
            body.play({ name: 'OFF', loop: false })
          }
        })
      ...
      if (playerId === player.id) {
        ...
        // Play engine ON animation on SkinnedMesh (loops as idle)
        safeExecute(() => {
          if (body && body.play) {
            body.play({ name: 'ON', loop: true })
          }
        })
    ...
    app.on('authority', playerId => {
      ...
      if (playerId === player.id) {
        setMode(simulateMode)
        // Show UI when gaining authority (always show for driver)
        if (speedometerUI) speedometerUI.active = true
        if (minimapUI) minimapUI.active = true
        if (minimapDot) minimapDot.active = true
        minimapGrid.forEach(gridLine => {
    ...
    app.on('update', delta => {
      ...
      // NEW: Update steering wheel and exhaust particles
      updateSteeringWheel()
      // Update exhaust particles for driver - more visible like simpleCar
      if (enableParticles) {
        const isEmitting = power > 0.2 && isEngineRunning
        const exhaustRate = power * 15 // Match simpleCar's visibility

        if (isEmitting && leftExhaustParticles && !leftExhaustParticles.emitting) {
          console.log('KART: Exhaust started emitting, rate:', exhaustRate)
        }

        if (leftExhaustParticles) {
          leftExhaustParticles.emitting = isEmitting
          leftExhaustParticles.rate = exhaustRate
        }
        if (rightExhaustParticles) {
          rightExhaustParticles.emitting = isEmitting
          rightExhaustParticles.rate = exhaustRate
        }
      }
...
function viewerMode() {
  ...
  return {
    ...
    update(delta) {
      ...
      // Update engine animation and exhaust for other players - exactly like simpleCar
      if (info.engineRunning) {
        safeExecute(() => {
          if (body && !body.playing) {
            body.play({ name: 'ON', loop: true })
          }
        })
      } else {
      ...
      // Update steering wheel rotation for other players
      safeExecute(() => {
        if (steeringWheel) {
          steeringWheel.rotation.y = info.steeringWheelRotation
        }
      })

      // Update exhaust particles with proper rate
      if (leftExhaustParticles && rightExhaustParticles) {
        const isEmitting = info.exhaustEmitting
        leftExhaustParticles.emitting = isEmitting
        rightExhaustParticles.emitting = isEmitting
        if (isEmitting) {
          const exhaustRate = 12 // Match driver's rate
          leftExhaustParticles.rate = exhaustRate
          rightExhaustParticles.rate = exhaustRate
        }
      }
      ...
      if (speedometerText && speedometerUI && speedometerUI.active) {
        // Get speed from physics or use existing speed variable
        const viewerSpeed = speed || 0
        const speedDisplay = Math.round(Math.abs(viewerSpeed))
  ...
  return {
    fixedUpdate(delta) {
      ...
      for (const wheel of wheels) {
        ...
        // Apply stronger braking when handbrake is engaged
        const frictionForce = wheel.rear && handbrakeInput ? handbrakeForce : regular
      ...
      for (const wheel of wheels) {
        ...
        if (handbrakeInput && wheel.rear) {
          gripFactor *= 0.1 // 90% grip reduction on rear wheels
        }
    ...
    update(delta) {
      if (control) {
        // Check for active joystick - EXACTLY like simpleCar
        ...
        // Hop input (spacebar on PC, button on mobile) - EXACTLY like simpleCar
        hopInput = control.space?.down || isHopButtonPressed

        if (isGasPressed) {
          // Gas button pressed: forward acceleration + steering
          accelInput = 1
          ...
          // Joystick controls both - EXACTLY like simpleCar
          steerInput = -stickX
          accelInput = -stickZ
        } else {
          // Keyboard only - EXACTLY like simpleCar
          steerInput = 0
          if (control.keyA?.down) steerInput += 1
          if (control.keyD?.down) steerInput -= 1
        ...
        // Handbrake removed - EXACTLY like simpleCar
        handbrakeInput = false
      }
