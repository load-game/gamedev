...
function loadClientHtmlTemplate(cache) {
  const candidates = [
    ...
    path.join(process.cwd(), 'packages/client/public/index.html'),
  ]
  for (const candidate of candidates) {
    try {
      if (!fs.existsSync(candidate)) continue
...
const bootstrapWorldPathOverride = bootstrapRuntimeEnabled && hasValue(process.env.WORLD) ? process.env.WORLD : null
...
async function handleBootstrapRequest(req, reply) {
  ...
  if (bootstrapWorldPathOverride) {
    candidateEnv.WORLD = bootstrapWorldPathOverride
  }
...
function registerCommonRoutes(app, { includeBootstrapControl = false, connectionChannel = 'main' } = {}) {
  ...
  app.post('/internal/admission/:action', async (req, reply) => {
    ...
    try {
      if (req.params.action === 'reserve') return admission.reserve(req.body?.sessionId, req.body?.identity)
      if (req.params.action === 'friend')
        return await runtimeState.resources.world.network.reserveFriend(
          req.body?.token,
          req.body?.sessionId,
          req.body?.identity
        )
