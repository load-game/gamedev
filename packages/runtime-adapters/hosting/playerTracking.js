...
export function createRuntimePlayerTracker({ hosting = null, world = null, env = process.env, logger = console } = {}) {
  const log = createLogger(logger)
  const label = hosting?.name || 'runtime-hosting'
  const canTrackPlayers =
    !!hosting &&
    typeof hosting.publishPlayerCapacity === 'function' &&
    typeof hosting.trackPlayerConnect === 'function' &&
    typeof hosting.trackPlayerDisconnect === 'function'
  ...
  async function publishCapacity(reason = 'update') {
    ...
    if (!Number.isInteger(nextCapacity) || nextCapacity <= 0) {
      if (capacityChanged) {
        log.info(`[${label}] skipped player capacity update (${reason})`)
      }
      return false
    }
    ...
    try {
      await hosting.publishPlayerCapacity(nextCapacity, { reason })
      lastPublishedCapacity = nextCapacity
      log.info(`[${label}] updated player capacity to ${nextCapacity} (${reason})`)
      return true
    } catch (err) {
      log.warn(`[${label}] failed to update player capacity to ${nextCapacity} (${formatErrorMessage(err)})`)
      return false
    }
  }
  ...
  async function trackPlayerConnect(playerId) {
    ...
    if (!normalizedPlayerId) {
      log.warn(`[${label}] skipped player connect with invalid player id`)
      return false
    }

    try {
      await hosting.trackPlayerConnect(normalizedPlayerId)
      return true
    } catch (err) {
      log.warn(`[${label}] failed to track player connect for ${normalizedPlayerId} (${formatErrorMessage(err)})`)
      return false
    }
  }
  ...
  async function trackPlayerDisconnect(playerId) {
    ...
    const normalizedPlayerId = normalizePlayerId(playerId)
    if (!normalizedPlayerId) {
      log.warn(`[${label}] skipped player disconnect with invalid player id`)
      return false
    }

    try {
      await hosting.trackPlayerDisconnect(normalizedPlayerId)
      return true
    } catch (err) {
      log.warn(`[${label}] failed to track player disconnect for ${normalizedPlayerId} (${formatErrorMessage(err)})`)
      return false
    }
  }
...
export const createAgonesPlayerTracker = createRuntimePlayerTracker
